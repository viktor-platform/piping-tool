...
from geolib.models.dseries_parser import DSerieListStructure
from geolib.models.dseries_parser import DSeriesInlineMappedProperties
from geolib.models.dseries_parser import DSeriesInlineReversedProperties
from geolib.models.dseries_parser import DSeriesNoParseSubStructure
...
from geolib.models.dseries_parser import DSeriesStructureCollection
from geolib.models.dseries_parser import DSeriesTableStructure
from geolib.models.dseries_parser import DSeriesUnmappedNameProperties
from geolib.models.dseries_parser import DSeriesWrappedTableStructure
from geolib.models.dseries_parser import DSerieVersion
...
from .calculation_options import CalculationOptionsPerStage as CalculationOptionsPerStageExternal
from .dsheetpiling_structures import DSeriesPilingDumpParserStructure
from .dsheetpiling_structures import DSeriesPilingParserStructure
from .dsheetpiling_structures import DSheetpilingSurchargeLoad
from .dsheetpiling_structures import DSheetpilingTableEntry
from .dsheetpiling_structures import DSheetpilingUnwrappedTable
from .dsheetpiling_structures import DSheetpilingWithNumberOfRowsTable
from .dsheetpiling_validator import DSheetPilingValidator
from .internal_partial_factors import PartialFactorsCurI
from .internal_partial_factors import PartialFactorsCurIi
from .internal_partial_factors import PartialFactorsCurIii
from .internal_partial_factors import PartialFactorsEc7BSet1
from .internal_partial_factors import PartialFactorsEc7BSet2
from .internal_partial_factors import PartialFactorsEc7Nl0
from .internal_partial_factors import PartialFactorsEc7Nl1
from .internal_partial_factors import PartialFactorsEc7Nl2
from .internal_partial_factors import PartialFactorsEc7Nl3
from .internal_partial_factors import PartialFactorsEc7SeVk1
from .internal_partial_factors import PartialFactorsEc7SeVk2
from .internal_partial_factors import PartialFactorsEc7SeVk3
from .internal_partial_factors import PartialFactorsEurocodeDa1Set1
from .internal_partial_factors import PartialFactorsEurocodeDa1Set2
from .internal_partial_factors import PartialFactorsEurocodeDa2
from .internal_partial_factors import PartialFactorsEurocodeDa3
from .settings import CalculationType
from .settings import CurveSettings
from .settings import DesignType
from .settings import DistributionType
from .settings import EarthPressureCoefficients
from .settings import GrainType
from .settings import HorizontalBehaviorType
from .settings import LambdaType
from .settings import LateralEarthPressureMethod
from .settings import LateralEarthPressureMethodStage
from .settings import LoadTypeFavourableUnfavourable
from .settings import LoadTypeFavourableUnfavourableMoment
from .settings import LoadTypePermanentVariable
from .settings import ModelType
from .settings import ModulusReactionType
from .settings import ModulusSubgradeReaction
from .settings import PartialFactorCalculationType
from .settings import PartialFactorSetCUR
from .settings import PartialFactorSetEC
from .settings import PartialFactorSetEC7NADB
from .settings import PartialFactorSetEC7NADNL
from .settings import PartialFactorSetVerifyEC
from .settings import PassiveSide
from .settings import SheetPilingElementMaterialType
from .settings import Side
from .settings import SinglePileLoadOptions
from .settings import SoilTypeModulusSubgradeReaction
from .settings import VerifyType
...
class SheetPileElement(DSeriesUnmappedNameProperties):
    ...
    def update_arguments_if_invalid_input(self) -> None:
        ...
        if self.sheetpilingelementkmod is not result_dict_k_mod_value[self.sheetpilingelementmaterialtype.value]:
            self.sheetpilingelementkmod = result_dict_k_mod_value[self.sheetpilingelementmaterialtype.value]
            warnings.warn(
                "Inputed value for kmod was not valid and was change to " + str(self.sheetpilingelementkmod),
                ImportWarning,
            )
        # gammma_m value
        ...
        if (
            ...
            self.sheetpilingelementmaterialfactor = result_dict_gamma_m_value[self.sheetpilingelementmaterialtype.value]
            warnings.warn(
...
class DSheetPilingInputStructure(DSeriesStructure):
    ...
    # Custom validator
    _validate_run_identification = make_newline_validator("run_identification", req_newlines=REQ_RUN_LINES)
    ...
    def set_model(self, **kwargs) -> None:
        ...
        if self.model.method != LateralEarthPressureMethod.MIXED:
            for stage in self.construction_stages.stages:
                stage.method_left = LateralEarthPressureMethodStage.get_stage_type_from_method(self.model.method)
                stage.method_right = LateralEarthPressureMethodStage.get_stage_type_from_method(self.model.method)
    ...
    def add_calculation_options_per_stage(
        ...
        stageoptions = StageOptions(**_map_external_to_internal_values[self.calculation_options.verifytype])
    ...
    def set_curve_settings(self, curve_settings: CurveSettings) -> None:
        ...
        self.soil_profiles.use_unloading_reloading_curve = curve_settings.use_unloading_reloading_curve
        self.soil_profiles.modulus_reaction_type = curve_settings.modulus_reaction_type.value
    ...
    def add_stage(
        ...
        if name in self.construction_stages.stage_names:
            raise ValueError(f"Stage name {name} already present: all stage names must be unique")
        ...
        self._validate_lateral_earth_pressure_methods_for_single_pile_model(method_left, method_right)
    ...
    def _get_validated_lateral_earth_pressure_method(
        ...
        error = ValueError(f"stage_method {stage_method} is not compatible with method on model {self.model.method}")
        if (
    ...
    def add_profile(self, stage_id: int, profile: SoilProfile, side: Side) -> None:
        ...
        for i, layer in enumerate(profile.layers):
            if layer.soil not in soil_names:
                raise ValueError(f"Soil {layer.soil} of layer {i} is not in soil collection; add soil first via model.")
    ...
    def set_construction(self, top_level: float, elements: List[SheetPileElement]) -> None:
    ...
    def add_anchor(self, stage_id: int, anchor: Anchor, pre_tension: float) -> None:
        ...
        self.construction_stages.stages[stage_id].anchors.append(
            AnchorOrStrutPresstressReference(name=anchor.name, pre_stress=pre_tension or _DEFAULT_PRE_STRESS)
        )
    ...
    def add_strut(self, stage_id: int, strut: Strut, pre_compression: float) -> None:
        ...
        self.construction_stages.stages[stage_id].struts.append(
            AnchorOrStrutPresstressReference(name=strut.name, pre_stress=pre_compression or _DEFAULT_PRE_STRESS)
        )

    def add_load(self, load: Union[HorizontalLineLoad, Moment, NormalForce], stage_id: int):
        if isinstance(load, HorizontalLineLoad):
            ...
            if isinstance(self.horizontal_line_loads, str) or self.horizontal_line_loads is None:
            ...
            self.construction_stages.stages[stage_id].horizontal_line_loads.append(load.name)
    ...
    def add_surcharge_load(self, stage_id: int, load: SurchargeLoad, side: Side) -> None:
        ...
        stage = self.construction_stages.stages[stage_id]
        if side == Side.BOTH:
            self.is_valid_unique_load_names(load_list=stage.surcharge_loads_left, name=load.name)
            stage.surcharge_loads_left.append(load.name)
            self.is_valid_unique_load_names(load_list=stage.surcharge_loads_right, name=load.name)
    ...
    def is_valid_unique_load_names(self, load_list: List[str], name: str) -> bool:
        ...
        if name in load_list:
            raise ValueError(f"{name} load name is duplicated. Please change the name of the load.")
        return True
...
class VerifySheetpileData(DSeriesStructure):
    ...
    verify_low_mod_with_alt_passive_waterlevel: Optional[BaseVerificationStructureProperties]
    verify_high_mod_with_alt_passive_waterlevel: Optional[BaseVerificationStructureProperties]
