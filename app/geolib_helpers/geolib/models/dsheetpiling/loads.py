...
class Moment(BaseDataClass):
    ...
    def to_internal(self) -> MomentInternal:
        moment = MomentInternal(**self.dict(exclude_none=True, exclude={"verification_load_settings"}))
...
class SurchargeLoad(BaseDataClass):
    ...
    def to_internal(self) -> InternalSurchargeLoad:
        surchargeload = InternalSurchargeLoad(
            name=self.name,
            points=[
                InternalSurchargePoint(surchargeloaddistance=point.x, surchargeloadvalue=point.z)
                for point in self.points
            ],
        )
        surchargeload.surchargeloadpermanent = self.verification_load_settings.duration_type
...
class NormalForce(BaseDataClass):
    ...
    def to_internal(self) -> NormalForceInternal:
        normalforce = NormalForceInternal(**self.dict(exclude_none=True, exclude={"verification_load_settings"}))
