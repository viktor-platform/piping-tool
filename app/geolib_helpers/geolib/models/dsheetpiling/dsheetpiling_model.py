...
from geolib.models.dsheetpiling.constructions import DiaphragmWall
from geolib.models.dsheetpiling.constructions import Pile
from geolib.models.dsheetpiling.constructions import Sheet
...
from .calculation_options import CalculationOptionsPerStage
...
from .internal import DSheetPilingDumpStructure
from .internal import DSheetPilingOutputStructure
from .internal import DSheetPilingStructure
from .internal import Water
from .loads import Earthquake
from .loads import HorizontalLineLoad
from .loads import Moment
from .loads import NormalForce
from .loads import SoilDisplacement
from .loads import SurchargeLoad
...
from .supports import Anchor
from .supports import RigidSupport
from .supports import SpringSupport
from .supports import Strut
...
class DSheetPilingModel(BaseModel):
    ...
    datastructure: Union[DSheetPilingStructure, DSheetPilingDumpStructure] = DSheetPilingStructure()
    ...
    def _is_calculation_per_stage_required(self) -> bool:
        ...
        if self.datastructure.input_data.calculation_options.inputcalculationtype == CalculationType.VERIFY_SHEETPILING:
            if (
                _map_method_b_available[self.datastructure.input_data.calculation_options.verifytype]
    ...
    def add_stage(
        ...
        self.datastructure.input_data.add_stage(name, passive_side, method_left, method_right, pile_top_displacement)
    ...
    def add_calculation_options_per_stage(
        ...
        if self._is_calculation_per_stage_required():
            ...
            raise ValueError(f"calculation_options_per_stage is not required for stage {stage_id}.")
    ...
    def _check_if_stage_id_exists(self, stage_id: int) -> None:
        if not self.datastructure.input_data.has_stage(stage_id):
            raise ValueError(f"Stage {stage_id} is not added to the internal datastructure")
    ...
    def set_model(self, model: BaseModelType) -> None:
        ...
        if not issubclass(type(model), BaseModelType):
            raise ValueError(f"model should be of subtype BaseModelType, received {model}")
    ...
    def set_calculation_options(self, calculation_options: CalculationOptions) -> None:
        ...
        if not issubclass(type(calculation_options), CalculationOptions):
            raise ValueError(f"model should be of subtype CalculationOptions, received {calculation_options}")
    ...
    def add_head_line(
        ...
        """
        self._check_if_stage_id_exists(stage_id)
        self.datastructure.input_data.add_water_level(stage_id, water_level.to_internal(), side=side)
    ...
    def add_surface(
        ...
        """
        self._check_if_stage_id_exists(stage_id)
        self.datastructure.input_data.add_surface(stage_id, surface.to_internal(), side=side)
    ...
    def add_profile(
        ...
        """
        self._check_if_stage_id_exists(stage_id)
        self.datastructure.input_data.add_profile(stage_id, profile.to_internal(), side=side)

    def set_construction(self, top_level: float, elements: List[Union[Sheet, DiaphragmWall, Pile]]) -> None:
    ...
    def add_surcharge_load(self, load: SurchargeLoad, side: Side, stage_id: int) -> None:
        ...
        """
        self._check_if_stage_id_exists(stage_id)
        self.datastructure.input_data.add_surcharge_load(stage_id, load.to_internal(), side=side)
    ...
    def add_anchor_or_strut(
        ...
        if isinstance(support, Anchor):
            self.datastructure.input_data.add_anchor(stage_id, support.to_internal(), pre_tension=pre_stress)
        elif isinstance(support, Strut):
            self.datastructure.input_data.add_strut(stage_id, support.to_internal(), pre_compression=pre_stress)
        else:
