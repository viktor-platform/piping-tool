...
from geolib.utils import camel_to_snake
...
class DStabilityBaseModelStructure(BaseModelStructure):
    def dict(_, *args, **kwargs):
        ...
        return {k: "NaN" if isinstance(v, float) and not isfinite(v) else v for k, v in data.items()}
...
class WaternetCreatorSettings(DStabilitySubStructure):
    ...
    ContentVersion: Optional[str] = "1"
    DitchCharacteristics: Optional[PersistableDitchCharacteristics] = PersistableDitchCharacteristics()
    DrainageConstruction: Optional[PersistablePoint] = PersistablePoint()
    EmbankmentCharacteristics: Optional[PersistableEmbankmentCharacteristics] = PersistableEmbankmentCharacteristics()
...
class SoilLayerCollection(DStabilitySubStructure):
    ...
    def get_ids(self, exclude_soil_layer_id: Optional[int]) -> Set[str]:
        ...
        return {layer.LayerId for layer in self.SoilLayers if layer.LayerId != exclude_soil_layer_id}
...
class PersistableSuTable(DStabilityBaseModelStructure):
    ...
    def to_global_su_table(self):
        ...
        for su_table_point in self.SuTablePoints:
            su_table.append(SuTablePoint(su=su_table_point.Su, stress=su_table_point.EffectiveStress))
        return su_table
...
class SoilCollection(DStabilitySubStructure):
    ...
    def __determine_strength_increase_exponent(self, persistable_soil: PersistableSoil):
        ...
        if (
            ...
            return self.__to_global_stochastic_parameter(persistable_soil.StrengthIncreaseExponentStochasticParameter)
        elif (
            persistable_soil.ShearStrengthModelTypeAbovePhreaticLevel.value == "SuTable"
            or persistable_soil.ShearStrengthModelTypeAbovePhreaticLevel.value == "SuTable"
        ):
    ...
    def __internal_soil_to_global_soil(self, persistable_soil: PersistableSoil):
        from geolib.soils import MohrCoulombParameters
        from geolib.soils import SoilWeightParameters
        from geolib.soils import UndrainedParameters

        mohr_coulomb_parameters = MohrCoulombParameters(
            cohesion=self.__to_global_stochastic_parameter(persistable_soil.CohesionStochasticParameter),
            friction_angle=self.__to_global_stochastic_parameter(persistable_soil.FrictionAngleStochasticParameter),
            dilatancy_angle=self.__to_global_stochastic_parameter(persistable_soil.DilatancyStochasticParameter),
            cohesion_and_friction_angle_correlated=persistable_soil.CohesionAndFrictionAngleCorrelated,
        )

        strength_increase_exponent = self.__determine_strength_increase_exponent(persistable_soil)
...
class Loads(DStabilitySubStructure):
    ...
    def add_load(
        ...
        if hasattr(internal_datastructure, "Consolidations"):
            internal_datastructure.Consolidations = [c.to_internal_datastructure() for c in consolidations]
...
class Geometry(DStabilitySubStructure):
    ...
    def add_layer(self, id: str, label: str, notes: str, points: List[Point]) -> PersistableLayer:
        """
...
class PersistableBishopBruteForceSettings(DStabilityBaseModelStructure):
    GridEnhancements: Optional[PersistableGridEnhancements] = PersistableGridEnhancements()
    SearchGrid: Optional[PersistableSearchGrid] = PersistableSearchGrid()
    SlipPlaneConstraints: Optional[PersistableSlipPlaneConstraints] = PersistableSlipPlaneConstraints()
...
class PersistableSpencerGeneticSettings(DStabilityBaseModelStructure):
    ...
    SlipPlaneConstraints: Optional[PersistableGeneticSlipPlaneConstraints] = PersistableGeneticSlipPlaneConstraints()
...
class PersistableUpliftVanSettings(DStabilityBaseModelStructure):
    SlipPlane: Optional[PersistableTwoCirclesOnTangentLine] = PersistableTwoCirclesOnTangentLine()
...
class CalculationSettings(DStabilitySubStructure):
    ...
    BishopBruteForce: Optional[PersistableBishopBruteForceSettings] = PersistableBishopBruteForceSettings()
    ...
    SpencerGenetic: Optional[PersistableSpencerGeneticSettings] = PersistableSpencerGeneticSettings()
...
class BishopBruteForceResult(DStabilitySubStructure):
    ...
    def get_slipcircle_output(self) -> BishopSlipCircleResult:
        ...
        try:
            return BishopSlipCircleResult(x=self.Circle.Center.X, z=self.Circle.Center.Z, radius=self.Circle.Radius)
...
class BishopReliabilityResult(DStabilitySubStructure):
    ...
    StateLinePointContributions: Optional[List[Optional[PersistableStateLinePointContribution]]] = None
    StatePointContributions: Optional[List[Optional[PersistableStatePointContribution]]] = None
...
class DStabilityStructure(BaseModelStructure):
    ...
    soillayers: List[SoilLayerCollection] = [SoilLayerCollection(Id="13")]  # soillayers/soillayers_x.json
    ...
    reinforcements: List[Reinforcements] = [Reinforcements(Id="19")]  # reinforcements/reinforcements_x.json
    ...
    decorations: List[Decorations] = [Decorations(Id="12")]  # decorations/decorations_x.json
    ...
    def duplicate_stage(self, current_stage: int, label: str, notes: str, unique_start_id: int):
        ...
        for fieldname, stagefield in self.get_stage_specific_fields(current_stage):
            ...
            unique_start_id = self.renumber_fk_fields(newstagefield, old_to_new, unique_start_id)
            # Renumber all children
            for classinstance in children(newstagefield):
                unique_start_id = self.renumber_fk_fields(classinstance, old_to_new, unique_start_id)
    ...
    def add_default_stage(self, label: str, notes: str, unique_start_id=500) -> int:
        ...
        self.waternetcreatorsettings += [WaternetCreatorSettings(Id=str(unique_start_id + 2))]
