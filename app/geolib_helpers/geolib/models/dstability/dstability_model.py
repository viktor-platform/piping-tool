...
from .internal import AnalysisType
from .internal import BishopSlipCircleResult
from .internal import CalculationType
from .internal import DStabilityResult
from .internal import DStabilityStructure
from .internal import PersistablePoint
from .internal import SoilCollection
from .internal import SpencerSlipPlaneResult
from .internal import Stage
from .internal import UpliftVanSlipCircleResult
from .internal import Waternet
from .loads import Consolidation
...
from .serializer import DStabilityInputZipSerializer
from .states import DStabilityStateLinePoint
from .states import DStabilityStatePoint
...
class DStabilityModel(BaseModel):
    ...
    def _get_result_substructure(self, stage_id: int) -> DStabilityResult:
        if self.datastructure.has_result(stage_id):
            ...
            results = self.datastructure.get_result_substructure(analysis_type, calculation_type)
    ...
    def add_stage(self, label: str, notes: str, set_current=True) -> int:
        ...
        new_stage_id, new_unique_id = self.datastructure.add_default_stage(label, notes, new_id)
    ...
    def copy_stage(self, label: str, notes: str, set_current=True) -> int:
        ...
        """
        new_id = self._get_next_id()
        new_stage_id, new_unique_id = self.datastructure.duplicate_stage(self.current_stage, label, notes, new_id)
    ...
    def add_layer(
        ...
        persistable_layer = geometry.add_layer(id=str(self._get_next_id()), label=label, points=points, notes=notes)
    ...
    def add_head_line(
        ...
        persistable_headline = waternet.add_head_line(str(self._get_next_id()), label, notes, points, is_phreatic_line)
    ...
    def add_load(
        ...
        if not issubclass(type(load), DStabilityLoad):
            raise ValueError(f"load should be a subclass of DstabilityReinforcement, received {load}")
    ...
    def _get_default_consolidations(
        ...
        if self.datastructure.has_soil_layers(stage_id):
            soil_layer_ids = self.datastructure.soillayers[stage_id].get_ids(exclude_soil_layer_id)
