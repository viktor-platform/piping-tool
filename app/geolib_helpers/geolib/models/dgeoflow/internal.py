...
class SoilCollection(DGeoflowSubStructure):
    ...
    Soils: List[PersistableSoil] = [
        ...
        PersistableSoil(
            Id="8", Name="Peat, deep", Code="H_vbv_v", HorizontalPermeability=1.157e-07, VerticalPermeability=1.157e-07
        ),
        PersistableSoil(
            Id="9",
            Name="Sand",
...
class BoundaryCondition(DGeoflowSubStructure):
    ...
    def add_boundarycondition(
        self, label: str, notes: str, points: List[Point], head_level: float
...
class Scenario(DGeoflowSubStructure):
    ...
    def add_stage(
        self, label: str, notes: str, layeractivation_collection_id: int, boundaryconditions_collection_id: int
    ) -> PersistableStage:
        ps = PersistableStage(
            Label=label,
            Notes=notes,
            LayerActivationCollectionId=layeractivation_collection_id,
...
class MeshProperty(DGeoflowSubStructure):
    ...
    def get_ids(self, exclude_soil_layer_id: Optional[int]) -> Set[str]:
        if exclude_soil_layer_id is not None:
            exclude_soil_layer_id = str(exclude_soil_layer_id)
        return {layer.LayerId for layer in self.MeshProperties if layer.LayerId != exclude_soil_layer_id}
...
class LayerActivation(DGeoflowSubStructure):
    ...
    def get_ids(self, exclude_soil_layer_id: Optional[int]) -> Set[str]:
        if exclude_soil_layer_id is not None:
            exclude_soil_layer_id = str(exclude_soil_layer_id)
        return {layer.LayerId for layer in self.LayerActivations if layer.LayerId != exclude_soil_layer_id}
...
class DGeoflowStructure(BaseModelStructure):
    ...
    soillayers: List[SoilLayerCollection] = [SoilLayerCollection(Id="14")]  # soillayers/soillayers_x.json
    ...
    def duplicate_scenario(self, current_stage: int, label: str, notes: str, unique_start_id: int):
    ...
    def add_default_scenario(self, label: str, notes: str, unique_start_id=500) -> int:
        ...
        self.scenarios += [Scenario(GeometryId=str(unique_start_id + 4), SoilLayersId=str(unique_start_id + 1))]
