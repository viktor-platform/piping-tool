...
from .internal import PersistableSoilLayer
from .internal import SoilCollection
from .internal import SoilLayerCollection
from .serializer import DGeoflowInputSerializer
from .serializer import DGeoflowInputZipSerializer
...
class DGeoflowModel(BaseModel):
    ...
    def add_scenario_2(self, label: str, notes: str, set_current=True) -> int:
        ...
        new_scenario_id, new_unique_id = self.datastructure.add_default_scenario(label, notes, new_id)
    ...
    def add_meshproperties(
        self, element_size: float = 1.0, label: str = "", scenario_id: int = None, layer_id: int = None
    ) -> int:
        """
    ...
    def add_boundarycondition(
        self, points: List[Point], head_level: float, label: str = "", notes: str = "", scenario_id: int = None
    ) -> int:
        """
    ...
    def add_scenario(
        ...
        scenarios.add_calculation(
            label=calculations_label, notes=calculations_notes, mesh_properties_id=meshproperties_id
        )
        scenarios.add_stage(
            label=stage_label,
            notes=stage_notes,
