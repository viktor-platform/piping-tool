...
from .utils import get_filtered_type_hints
from .utils import get_required_class_field
from .utils import is_list
from .utils import is_union
from .utils import unpack_if_union
        ...
        for field, fieldtype in get_type_hints(self).items():
            ...
            # whether we can parse it further.
                ...
                if is_list(fieldtype):
                    ...
                    # Check whether we can parse and if so, parse the strings
                    if hasattr(fieldtype, "is_parseable") and fieldtype.is_parseable():
                        logger.debug(f"Changed {field} to single {fieldtype} for list of str.")
                        kwargs[field] = fieldtype.parse_text(body)
                    else:
                        if DataClass in fieldtype.__mro__:
                            logger.debug(f"Can't parse {fieldtype} for {field} yet")
    ...
    def dict(_, *args, **kwargs):
        ...
        for __, value in data.items():
            if isinstance(value, float) and not isfinite(value):
                raise ValueError("Only finite values are supported, don't use nan, -inf or inf.")
        return data
...
class DSerieListStructure(DSeriesStructure):
    ...
    def parse_text(cls, text: str):
        ...
        Arguments:
            data {str} -- Data to parse.
        """
        lines = [strip_line_first_element(line) for line in text.split("\n") if line != ""]
...
class DSeriesInlineProperties(DSeriesStructure):
    ...
    def get_properties_in_text(cls, text: str):
        required_properties_names = [structure_name for structure_name, _ in cls.get_structure_required_fields()]
        header_lines = cls.header_lines()
        for idx, (key, value) in enumerate(DSerieParser.parse_group(text, loose_properties=True)):
            if idx < header_lines:
                continue
            if not key:
                # Only loose properties would get through here.
                # So a grouped property enclosed like [PROPERTY][END OF PROPERTY].
                key, value = cls.get_property_key_value(value, required_properties_names[idx - header_lines])
            yield key, value
...
class DSeriesStructureCollection(DSeriesStructure):
    ...
    def parse_text(cls, text: str) -> DSeriesStructure:
        ...
        if len(fields) != 1:
            raise ValueError(f"This type of collection is only meant to have one field but {len(fields)} were defined.")

        parsed_groups = [text_value for _, text_value in DSerieParser.parse_group(text, loose_properties=True)]
        number_of_groups = get_line_property_value(parsed_groups.pop(0), reversed_key=True)

        if len(parsed_groups) != int(number_of_groups):
            raise ValueError(f"Expected {number_of_groups} groups, but parsed {len(parsed_groups)}.")
...
class DSeriesRepeatedGroupedProperties(DSeriesStructure):
    ...
    def get_list_field_names(cls) -> List[str]:
        ...
        """
        return [field_name for field_name, field_type in get_type_hints(cls).items() if is_list(field_type)]
...
class DSeriesRepeatedGroupsWithInlineMappedProperties(DSeriesRepeatedGroupedProperties):
    ...
    def get_inline_properties(cls, inline_properties: List[str]) -> Dict[str, str]:
        ...
        for unmapped in inline_properties:
            unformatted_key, value = get_line_property_key_value(unmapped, reversed_key=False)
            key = make_key(unformatted_key)
            ...
            if key in result_dict:
                logger.debug(f"Key {key} already mapped, value {value} will not be mapped.")
                continue
...
class DSeriesTreeStructure(DSeriesStructure):
    ...
    @classmethod
    def parse_text_lines(cls, text_lines: List[List[str]]) -> Tuple[DSeriesStructure, int]:
        ...
        def get_list_values(struct_idx: int, field_name: str, text_lines: list) -> Tuple[DSeriesStructure, int]:
        ...
        if len(text_lines) < len(required_fields):
            raise ValueError(
                f"There should be at least {len(required_fields)}" + f" fields to correctly initalize object {cls}"
            )
        lines_read = 0
        for struct_idx, (field_name, field) in enumerate(required_fields):
            ...
            if is_list(field) or issubclass(field, list) or is_structure_collection(field):
                ...
                properties[field_name] = get_line_property_value(text_lines[lines_read][0], reversed_key=True)
    ...
    @classmethod
    def get_tree_structure_read_lines(cls, parsed_tuple: Tuple[DSeriesStructure, int]) -> Tuple[DSeriesStructure, int]:
...
class DSeriesTreeStructurePropertiesInGroups(DSeriesTreeStructure):
    ...
    def get_next_property_text_lines(cls, text_lines: list) -> list:
        ...
        """
        filtered_property_lines = [split_line_elements(line) for line in text_lines[0][0].split("\n") if line != ""]
...
class DSeriesTreeStructureCollection(DSeriesStructure):
    ...
    @classmethod
    def parse_collection_type(cls, collecion_type: Type, lines: list) -> Tuple[DSeriesStructure, int]:
    ...
    def parse_text_lines(cls, lines: List[str]) -> Tuple[DSeriesStructure, int]:
        ...
        for _ in range(number_of_structures):
            ...
            parsed_structure, parsed_lines = cls.parse_collection_type(structure_type, lines[read_lines:])
...
class DSeriesMatrixTreeStructureCollection(DSeriesTreeStructureCollection):
    ...
    @classmethod
    def parse_collection_type(cls, collecion_type: Type, lines: list) -> Tuple[DSeriesStructure, int]:
        parsed_structure, _ = collecion_type.parse_text_lines([[line] for line in lines[0]])
        ...
        for i, line in enumerate(text_lines):
            ...
            if sline.startswith("[") and sline.endswith("]"):
                ...
                # duplicate group before end
                    ...
                    raise ValueError(f"Can't parse duplicate key at line {i} without first encountering and END OF.")
...
def read_property_as_list(field_name: str, field: Type, text_lines: list) -> Tuple[List[DSeriesStructure], int]:
