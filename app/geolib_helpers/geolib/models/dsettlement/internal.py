...
from geolib.models.dseries_parser import DSerieMatrixStructure
from geolib.models.dseries_parser import DSerieOldTableStructure
from geolib.models.dseries_parser import DSeriesInlineMappedProperties
from geolib.models.dseries_parser import DSeriesMatrixTreeStructureCollection
...
from geolib.models.dseries_parser import DSeriesTreeStructureCollection
...
from geolib.models.dsettlement.dsettlement_structures import ComplexVerticalSubstructure
from geolib.models.dsettlement.dsettlement_structures import DSerieRepeatedTableStructure
from geolib.models.dsettlement.internal_soil import SoilInternal
from geolib.models.dsettlement.probabilistic_calculation_types import ProbabilisticCalculationType
...
from geolib.soils import HorizontalBehaviourType
...
from .drain_types import DrainGridType
from .drain_types import DrainSchedule
from .drain_types import DrainType
...
class DSeriePoint(DSeriesTreeStructure):
    ...
    def get_structure_required_fields(cls) -> List[Tuple[str, Type]]:
        point_required_field_names = [required_field[0] for required_field in get_required_class_field(cls)]
        return [
            field_tuple
...
class Curves(DSeriesTreeStructureCollection):
    ...
    def create_curve(self, a: DSeriePoint, b: DSeriePoint):
        ...
        if curve in self.curves:
            ...
            new_curve_id = max(self.curves, key=attrgetter("id")).id + 1 if len(self.curves) > 0 else 1
    ...
    def create_curves(self, sorted_points: List[DSeriePoint]) -> List[Curve]:
        ...
        new_curves = [self.create_curve(sorted_points[i], sorted_points[i + 1]) for i in range(len(sorted_points) - 1)]
        return new_curves
...
class Boundaries(DSeriesTreeStructureCollection):
    ...
    def create_boundary(self, curves: List[Curve]) -> Boundary:
        ...
        if boundary in self.boundaries:
            ...
            new_boundary_id = max(self.boundaries, key=attrgetter("id")).id + 1 if len(self.boundaries) > 0 else 0
...
class Layer(DSeriesTreeStructure):
    ...
    def __eq__(self, other: object) -> bool:
        ...
        if not isinstance(other, Layer):
            ...
            return self.boundary_top == other.boundary_top and self.boundary_bottom == other.boundary_bottom
...
class Layers(DSeriesTreeStructureCollection):
    ...
    def add_layer(self, layer: Layer):
        ...
        if layer in self.layers:
            ...
            logger.warning(f"It's not possible to replace existing layers: {existing_layer}")
            return existing_layer
        else:
            new_layer_id = max(self.layers, key=attrgetter("id")).id + 1 if len(self.layers) > 0 else 1
...
class PiezoLines(DSeriesTreeStructureCollection):
    ...
    def create_piezoline(self, curves: List[Curve]) -> PiezoLine:
        ...
        if piezoline in self.piezolines:
            ...
            new_piezoline_id = max(self.piezolines, key=attrgetter("id")).id + 1 if len(self.piezolines) > 0 else 1
...
class UseProbabilisticDefaultsBoundaries(DSerieListStructure):
    ...
    def append_use_probabilistic_defaults_boundary(self, use_probabilistic_boundary: bool):
        self.useprobabilisticdefaultsboundaries.append(Bool(int(use_probabilistic_boundary)))
...
class DistributionBoundaries(DSerieListStructure):
    ...
    def append_distribution_boundary(self, distribution_boundary: DistributionType):
        if (distribution_boundary is DistributionType.Normal) or (distribution_boundary is DistributionType.Undefined):
...
class GeometryData(DSeriesStructure):
    ...
    def sort_boundaries(self):
        self.boundaries.boundaries.sort(key=lambda boundary: self.boundary_area_above_horizontal(boundary))
        ...
        for i, boundary in enumerate(self.boundaries.boundaries):
            ...
            if boundary.id != i:
                logger.warning(f"Boundary id {boundary.id} has changed to {i} in sorting process")
            boundary.id = i
...
class NonUniformLoads(DSeriesNoParseSubStructure):
    ...
    def add_load(self, name: constr(min_length=1, max_length=25), load: NonUniformLoad) -> Optional[ValueError]:
...
class WaterLoads(DSeriesNoParseSubStructure):
    ...
    def add_waterload(self, name: str, time: int, phreatic_line: int, headlines: List[List[int]]):
        self.waterloads.append(WaterLoad(name=name, time=time, phreatic_line=phreatic_line, headlines=headlines))
...
class OtherLoads(DSeriesNoParseSubStructure):
    ...
    def add_load(self, name: constr(min_length=1, max_length=25), load: OtherLoad) -> Optional[ValueError]:
        if name in self.loads:
            raise ValueError(f"Load with name '{name}' already exists.")
        else:
            self.loads[name] = load
...
class VerticalDrain(DSeriesNoParseSubStructure):
    ...
    water_head_during_dewatering: confloat(ge=-10000000.000, le=10000000.000) = 0  # relevant for strip and column
    tube_pressure_during_dewatering: confloat(ge=-10000000.000, le=10000000.000) = 0  # relevant for the sand wall
...
class ProbabilisticData(DSeriesInlineMappedProperties):
    ...
    def set_probabilistic_data(
        ...
        if reliability_type == ProbabilisticCalculationType.SettlementsDeterministic and is_reliability_calculation:
            raise ValueError(
        ...
        return ProbabilisticData(
            ...
            reliability_type=self.externaltointernalprobabilisticcalculationtype(reliability_type),
...
class DSettlementStructure(DSeriesStructure):
    ...
    def check_x_in_vertical(self, point_of_vertical: Point):
        ...
        if not (check_that_x_in_verticals):
            raise ValueError(f"The x-coordinate of point_of_vertical does not correspond to an existing vertical.")
