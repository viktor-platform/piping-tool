...
from geolib.models.dseries_parser import DSerieListGroupNextStructure
...
from geolib.models.dseries_parser import get_line_property_key_value
from geolib.models.dseries_parser import get_line_property_value
from geolib.models.dseries_parser import make_key
from geolib.models.dseries_parser import split_line_elements
from geolib.models.dseries_parser import strip_line_first_element
        ...
        for i, line in enumerate(text_lines):
            ...
            if sline.startswith("[") and sline.endswith("]"):
                ...
                # duplicate group before end
                    ...
                    logger.debug(f"Duplicate key {key} at line {i} without first encountered and END OF.")
...
class DSerieRepeatedTableStructure(DSeriesRepeatedGroupedProperties):
    ...
    def get_validated_mappings(cls, generated_dict: Dict[str, str]) -> Dict[str, str]:
        ...
        columns = [make_key(line) for line in generated_dict.pop("column_indication").split("\n") if line != ""]
        ...
        for group_data in list(generated_dict.values())[0]:
            ...
            if group_key in parsed_dict.keys():
                raise ValueError(f"No repeated table keys ({group_key}) allowed for {class_name}.")
            parsed_dict.update(mapped_group)

        return {cls.__name__.lower(): parsed_dict}

    @staticmethod
    def get_mapped_group(column_list: List[str], group_line: str) -> Dict[str, List[Dict[str, str]]]:
        ...
        """
        lines = [split_line_elements(line) for line in group_line.split("\n") if line != ""]
