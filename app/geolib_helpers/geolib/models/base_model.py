...
class BaseModel(BaseDataClass, abc.ABC):
    ...
    def execute(self, timeout_in_seconds: int = meta.timeout) -> "BaseModel":
        ...
        logger.info(f"Checking for {output_filename}, while process exited with {process.returncode}")
        if output_filename.exists():
            try:
                ...
                logger.warning(f"Ouput file generated but parsing of {output_filename} failed.")
    ...
    def execute_remote(self, endpoint: HttpUrl) -> "BaseModel":
        ...
        response = requests.post(
            requests.compat.urljoin(endpoint, f"calculate/{self.__class__.__name__.lower()}"),
            data=self.json(),
    ...
    @abstractmethod
    def serialize(self, filename: Union[FilePath, DirectoryPath, None]) -> Union[FilePath, DirectoryPath, None]:
...
class BaseModelList(BaseDataClass):
            ...
            for model in models:
                ...
                if output_filename.exists():
                    try:
                        ...
                        logger.warning(f"Ouput file generated but parsing of {output_filename.name} failed.")
    ...
    def execute_remote(self, endpoint: HttpUrl) -> "BaseModelList":
        ...
        response = requests.post(
            requests.compat.urljoin(endpoint, f"calculate/{lead_model.__class__.__name__.lower()}s"),
