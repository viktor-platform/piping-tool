...
def make_newline_validator(*field_name: str, req_newlines: int = 2):
    ...
    def field_must_contain_newlines(v: str):
        ...
        if newlines < req_newlines:
            logger.warning(f"Added {req_newlines - newlines} lines to run_identification.")
            v += (req_newlines - newlines) * "\n"
        elif newlines > req_newlines:
            logger.warning(f"More than {req_newlines+1} lines in run_identification will be ignored in the GUI.")
        return v
