...
class ShearStrengthModelTypePhreaticLevel(Enum):
    ...
    def transform_shear_strength_model_type_to_internal(self):
        from geolib.models.dstability.internal import ShearStrengthModelTypePhreaticLevelInternal
    ...
    def to_su_table_points(self):
        ...
        if self.su_table is not None:
            ...
            for su_point in self.su_table:
                su_table.append(PersistableSuTablePoint(EffectiveStress=su_point.stress, Su=su_point.su))
        return su_table
...
class StorageParameters(SoilBaseModel):
    ...
    permeability_strain_type: Optional[Union[float, StochasticParameter]] = StochasticParameter(mean=1e15)
...
class SoilClassificationParameters(SoilBaseModel):
    ...
    grain_type: Optional[GrainType] = GrainType.FINE  # TODO this must refer to a intenum class
...
class Soil(SoilBaseModel):
    ...
    soil_classification_parameters: Optional[SoilClassificationParameters] = SoilClassificationParameters()
    soil_stiffness_parameters: Optional[SoilStiffnessParameters] = SoilStiffnessParameters()
    ...
    shear_strength_model_above_phreatic_level: Optional[ShearStrengthModelTypePhreaticLevel] = None
    shear_strength_model_below_phreatic_level: Optional[ShearStrengthModelTypePhreaticLevel] = None
    ...
    def __transfer_soil_dict_to_model(soil_dict, model_soil):
        ...
        """
        for key, value in dict(soil_dict).items():  # override default values with those of the soil
    ...
    def _to_dstability(self):
        ...
        if self.shear_strength_model_above_phreatic_level is not None:
            ...
            shear_strength_model_above_phreatic_level = self.shear_strength_model_above_phreatic_level
        if self.shear_strength_model_below_phreatic_level is not None:
            ...
            shear_strength_model_below_phreatic_level = self.shear_strength_model_below_phreatic_level
    ...
    def _to_dsheetpiling(self):
        ...
        from geolib.models.dsheetpiling.settings import EarthPressureCoefficients
        from geolib.models.dsheetpiling.settings import SoilTypeModulusSubgradeReaction
        ...
        return DSheetPilingSoil(
            name=self.name,
            soilcolor=self.color.to_internal(),
            soilsoiltype=SoilTypeModulusSubgradeReaction(self.soil_type_settlement_by_vibrations.value),
