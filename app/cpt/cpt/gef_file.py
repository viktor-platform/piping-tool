...
class GEFFile(SDKGEFFile):
    ...
    def convert_to_imbro_file_content(self, ADDITIONAL_CLASSIFICATION_COLUMNS=None) -> bytes:
        """Converts the GEFFile data to an IMBRO XML file."""
        cpt_dict = munchify(self.parse(additional_columns=ADDITIONAL_CLASSIFICATION_COLUMNS, return_gef_data_obj=False))
        ...
        try:
            data = {
                ...
                "registrationHistory": {
                    "objectRegistrationTime": time_str[:-2]
                    + ":"
                    + time_str[-2:],  # Fixes format of strftime for timezone
    ...
    def _reformat_using_template(cls, data: Munch, template: dict, skip_root: bool = True):
        ...
        if skip_root:
            ...
            filled_template[root_tag]["value"] = cls._reformat_using_template(
                data, template[root_tag]["value"], skip_root=False
            )
        else:
            for tag, value in data.items():
                ...
                if isinstance(value, dict):
                    filled_template[tag_in_template]["value"] = cls._reformat_using_template(
                        value, template[tag_in_template]["value"], skip_root=False
                    )
                else:
