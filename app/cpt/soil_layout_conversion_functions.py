...
def convert_soil_layout_from_mm_to_m(soil_layout: SoilLayout) -> SoilLayout:
    """Converts the units of the TNOSoilLayout from mm to m."""
...
def convert_soil_layout_from_m_to_mm(soil_layout: SoilLayout) -> SoilLayout:
    """Converts the units of the TNOSoilLayout from m to mm."""
    serialization_dict = soil_layout.serialize()
    for layer in serialization_dict["layers"]:
        layer["top_of_layer"] = layer["top_of_layer"] * 1000
        layer["bottom_of_layer"] = layer["bottom_of_layer"] * 1000
...
def convert_input_table_field_to_soil_layout(
    bottom_of_soil_layout_user: float, soil_layers_from_table_input: List[dict], soils: Dict[str, Soil]
) -> SoilLayout:
    """Creates a TNOSoilLayout from the user input.
...
def convert_soil_layout_to_input_table_field(soil_layout: SoilLayout) -> List[dict]:
    """Converts a TNOSoilLayout to the parametrisation representation (Field = InputTable).

    :param soil_layout: TNOSoilLayout
...
class Classification:
    ...
    def classify_cpt_file(self, cpt_file: Union[GEFFile, IMBROFile], saved_ground_water_level=None) -> dict:
        ...
        try:
            ...
            # Classify the CPTData object to get a TNOSoilLayout
        ...
        cpt_dict["tno_soil_layout"] = convert_soil_layout_to_input_table_field(soil_layout_filtered_in_m)
...
def convert_table_to_soil_layers_2d(
    soil_layout_table: List[dict],
    columns: List[str],
    bottoms: List[float],
    positions: List[float],
    soils: dict,
    phreatic_levels: List[float],
):
    ...
    pl_line_bottom = PiezoLine(
        [Point(positions[0], bottoms[0] * 1e3), Point(positions[-1], bottoms[-1] * 1e3)], phreatic=False
    )
    pl_line_top = PiezoLine(
        [
            Point(positions[0], r[columns[0]] * 1e3 if r[columns[0]] else bottom_profile[0].y),
    ...
    for row in soil_layout_table[::-1]:
        ...
        top_profile = [
            Point(position, row[column] * 1e3 if row[column] else bottom_profile[i].y)
            for i, (position, column) in enumerate(zip(positions, columns))
        ]

        if any(
            pt_bot.y <= phreatic_level * 1e3 <= pt_top.y
            for pt_top, pt_bot, phreatic_level in zip(top_profile, bottom_profile, phreatic_levels)
        ):
            piezo_line = PiezoLine(
                [
                    Point(x, y)
                    for x, y in [(positions[0], phreatic_levels[0] * 1e3), (positions[-1], phreatic_levels[-1] * 1e3)]
                ],
                phreatic=first_phreatic,
            )

            first_phreatic = False
            soil_layers_2d.append(
                SoilLayer2D(soil, PolylineSDK(top_profile), PolylineSDK(bottom_profile), piezo_line_top=piezo_line)
            )
        else:
